...
pub fn moviebox_play_info_json_to_releases(
    ...
    for stream in streams {
        ...
        if !sign_cookie.is_empty() {
            let mut clean_cookie = sign_cookie
                .trim_end_matches(';')
                .split(';')
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .collect::<Vec<_>>()
                .join("; ");
            if clean_cookie.contains("CloudFront-Policy") && !clean_cookie.contains("CloudFront-Key-Pair-Id") {
                clean_cookie.push_str("; CloudFront-Key-Pair-Id=KMHN1LQ1HEUPL");
            }
