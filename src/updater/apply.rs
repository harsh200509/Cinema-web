    ...
    match env {
        InstallationEnvironment::Homebrew => {
            Ok(SelfUpdateOutcome::RequiresManualUpgrade(
                "This installation is managed by Homebrew. Run: brew upgrade cinema".to_string(),
            ))
        }
        ...
        InstallationEnvironment::Snap => {
            Ok(SelfUpdateOutcome::RequiresManualUpgrade(
                "Running inside Snap. Please update via: sudo snap refresh cinema".to_string(),
            ))
        }
...
mod tests {
    ...
    fn persistent_staging_path_lives_beside_executable() {
        let exe = Path::new("/opt/tools/cinema.exe");
    ...
    fn stale_artifacts_cover_staging_and_helper() {
        let exe = Path::new("C:/App/cinema.exe");
    ...
    fn helper_script_waits_moves_and_cleans_up() {
        ...
        assert!(script.contains(
            "move /y \"C:\\App\\update dir\\.moviebox_update_staged.exe\" \"C:\\App\\cinema.exe\" >NUL 2>&1"
        ));
        ...
        assert!(script.contains("start \"\" \"C:\\App\\cinema.exe\""));
