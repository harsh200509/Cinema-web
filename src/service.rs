...
impl MovieBoxService {
    ...
    pub async fn streams_typed(
        ...
        match provider {
            ProviderKind::MovieBox => ReleaseProvider::episode_streams(&self.client, subject_id, season, episode).await,
            ProviderKind::FourKHdHub => {
                let fourk = self.fourk_client.as_ref().ok_or_else(|| {
                    ProviderError::Unavailable("4KHDHub is unavailable".to_string())
                })?;
            ...
            ProviderKind::BdixCircleFtp => {
                ReleaseProvider::episode_streams(&self.circleftp_client, subject_id, season, episode).await
            }
            ProviderKind::BdixDhakaFlix => {
                ReleaseProvider::episode_streams(&self.dhakaflix_client, subject_id, season, episode).await
            }
            ProviderKind::Addons => {
                let addons = crate::config::load_addons();
                let is_series = season > 0;
